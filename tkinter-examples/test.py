...
canvas = Canvas(frame, width=800, height=500, bd=0, highlightthickness=0)
canvas.pack()
...
dot = canvas.create_oval(1,1,5,5, fill = "black")
...
try:
    while 1:
        ...
        canvas.scale("a",1,1,1.01,1)
        root.update() # process events
        root.after(30)
except TclError:
    traceback.print_exc() # to avoid errors when the window is closed
